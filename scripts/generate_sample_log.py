...
-----------------------
Seeds logs/orders_log.txt with three sample orders using the same
core functions the live app uses (no duplicated logic).

Run from the project root with:
    python scripts/generate_sample_log.py

Three sample carts:
  1. Single pizza, no toppings, qty 1 (no discount) -- Card
  2. Multi-pizza cart hitting the 5-pizza discount, with multiple
     toppings (different quantities) on one line -- UPI
  3. Two different pizzas with different bases, one topping each,
     total under 5 (no discount) -- Cash
"""
...
from config.settings import DEFAULT_LOG_PATH
...
def b(i): return base[i - 1]
def p(i): return pizza[i - 1]
def t(i): return topping[i - 1]
