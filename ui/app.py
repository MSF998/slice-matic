...
app.py
------
SliceMatic / PizzaFlow ordering system -- Gradio UI (Stage 2, v2).

This file is ONLY the UI layer. All validation, pricing, and persistence
rules live in core.pizzaflow_core and are tested independently with no
Gradio involved (tests/test_core.py).

PRODUCT MODEL (v2 -- cart-based)
----------------------------------
One card per pizza loaded from data/Types_of_Pizza.txt. Each card has:
  - a photo (hotlinked from Wikimedia Commons)
  - its own base/crust dropdown
  - its own quantity (0 = skip this pizza)
  - its own bank of topping quantity boxes (one per topping, each 0-5)

Images require internet access; if offline, the img tags show blank but
everything else continues to work normally.
"""

import gradio as gr

import core.pizzaflow_core as core
from config.settings import (
    NAVY, PINK,
    PIZZA_IMAGES, FALLBACK_IMAGE,
    LINE_QTY_MAX, TOPPING_QTY_MAX,
)
...
# ----------------------------------------------------------------------
# Load menus once at startup. If this fails, the ordering flow stays
# hidden behind a fatal-error panel.
# ----------------------------------------------------------------------
...
def build_order_continue(*args):
    ...
    Validates every field through core (Gradio component min/max are
    only UX hints; the real enforcement lives in pizzaflow_core).
    """
    try:
        state       = args[-1]
        rest        = args[:-1]
        ...
        for i in range(P):
            ...
            if qty == 0:
                continue  # pizza not being ordered -- skip base/topping validation
...
def payment_continue(payment_raw, state):
    try:
        ...
        record = core.build_cart_order_record(
            state["name"], state["phone"], state["bill"], idx,
...
# theme and css passed to launch() in Gradio 6+ (moved from Blocks constructor)
LAUNCH_KWARGS = {"theme": THEME, "css": CUSTOM_CSS}

with gr.Blocks(title="SliceMatic -- PizzaFlow Ordering") as demo:
    ...
    fatal_panel = gr.Markdown(
        value=(
            f"## \u274c Could not start SliceMatic\n\n{FATAL_ERROR}\n\n"
            "Fix the menu file(s) and restart the app."
        ) if FATAL_ERROR else "",
    ...
    with gr.Column(visible=not FATAL_ERROR):
        ...
        with gr.Group(visible=False) as step_build:
            ...
            gr.Markdown(
                "Set a quantity on any pizza you want. Pick its base and add toppings "
                "(with their own quantity) as you like. Leave quantity at 0 to skip a pizza."
            )
                ...
                with gr.Group(elem_classes="sm-card"):
                    with gr.Row():
                        with gr.Column(scale=1, min_width=140):
                            gr.HTML(
                                f'<img src="{image_for(pizza["name"])}" '
                                f'style="width:100%;max-height:140px;object-fit:cover;border-radius:10px;" '
                                f'alt="{pizza["name"]}">'
                            )
                        with gr.Column(scale=3):
                            ...
                            with gr.Row():
                                base_dd  = gr.Dropdown(
                                    choices=BASE_LABELS,
                                    value=BASE_LABELS[0] if BASE_LABELS else None,
                                    label="Base / Crust",
                                )
                                qty_num  = gr.Number(
                                    value=0, minimum=0, maximum=LINE_QTY_MAX,
                                    step=1, precision=0, label="Quantity",
                                )
                                ...
                                for chunk_start in range(0, len(TOPPINGS), chunk_size):
                                    ...
                                    with gr.Row():
                                        for topping in chunk:
                                            t_num = gr.Number(
                                                value=0, minimum=0, maximum=TOPPING_QTY_MAX,
                                                step=1, precision=0,
    ...
    intake_btn.click(
        intake_continue,
        inputs=[name_in, phone_in, order_state],
        outputs=[step_intake, step_build, intake_err, order_state],
    )
