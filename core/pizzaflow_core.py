...
All magic variables (constants, paths, pricing rules, log schema) are
imported from config.settings -- nothing is hard-coded here.

PRODUCT MODEL (v2 -- cart-based)
---------------------------------
The order is a CART of pizza "lines". Each line is one pizza from the
menu with:
...
from config.settings import (
    NAME_MIN_LEN, NAME_MAX_LEN, NAME_RE,
    PHONE_LEN, PHONE_VALID_START,
    LINE_QTY_MIN, LINE_QTY_MAX,
    TOTAL_QTY_MAX,
    TOPPING_QTY_MIN, TOPPING_QTY_MAX,
    DISCOUNT_THRESHOLD, DISCOUNT_RATE, GST_RATE,
    PAYMENT_MODES,
    DEFAULT_LOG_PATH, LOGS_DIR,
    ORDER_FIELD_ORDER, LINE_FIELD_ORDER,
    MENU_FILES,
)
...
def parse_strict_int(raw):
    ...
    Accepts raw strings (test_core exercises these directly) and Python
    int/float (what Gradio's gr.Number hands the callback). A float is
    only accepted if it is a whole number -- 2.0 -> 2, but 2.5 still
    raises, which is the "reject decimals" rule the brief asks for.
    """
    if raw is None:
...
def coerce_int(value, default=0):
    """
    Best-effort int conversion for Gradio Number components (which may
    hand back float, int, None, or "" depending on the browser). Falls
    back to `default` rather than raising; the caller's validate_*()
    still applies the real business-rule checks.
    """
...
def load_menu_file(path):
    ...
    if skipped:
        warning = (
            f"Skipped {len(skipped)} bad line(s) in '{os.path.basename(path)}': "
            + "; ".join(skipped)
        )
...
def validate_line_quantity(raw_qty):
    """
    Validates the quantity box next to ONE pizza row. 0 is valid (means
    'not ordering this pizza'). The old 'must be >= 1' rule now only
    applies to the ORDER TOTAL (see validate_order_total).
    """
...
def validate_topping_quantity(raw_qty):
    """Validates one topping's quantity for one pizza row. 0 = not added."""
    if raw_qty is None or str(raw_qty).strip() == "":
        return True, "", 0  # blank topping box means 0, not an error
    try:
        qty = parse_strict_int(raw_qty)
    except ValueError:
...
def validate_order_total(total_qty):
    ...
    the whole cart. This is where 'must order at least 1' and the 10-pizza
    capacity ceiling live.
    """
    if total_qty < 1:
...
def compute_line(pizza, base, quantity, toppings_with_qty):
    """
    Price breakdown for one pizza row.

    pizza, base           : menu item dicts {"id","name","price"}
    quantity              : validated int >= 0
    toppings_with_qty     : list of (topping_dict, qty) pairs, qty >= 1
                            (callers filter out qty == 0 toppings before calling)

    Topping cost model: topping price * topping_qty is the per-unit
    cost, then multiplied by the number of pizzas on this row. e.g.
    quantity=3, Extra Cheese qty=2 => 3 * 2 * extra_cheese_price added
    because each of the 3 pizzas gets double cheese.
    """
...
def compute_cart_bill(lines):
    """
    lines: list of compute_line() results, filtered to quantity > 0.

    Discount (10%) applied on the pre-GST subtotal once the TOTAL pizza
    count across all lines reaches DISCOUNT_THRESHOLD. GST always
...
def bill_to_rows(bill):
    """Render the full cart bill as rows for a gr.Dataframe."""
    rows = []
    for line in bill["lines"]:
        pizza, base, qty = line["pizza"], line["base"], line["quantity"]
        topping_desc = (
            ", ".join(f"{item['name']} x{q}" for item, q in line["toppings_with_qty"])
            if line["toppings_with_qty"] else "(no toppings)"
        )
        rows.append([
            pizza["name"],
...
def append_order(record, log_path=DEFAULT_LOG_PATH):
    """
    Append one order record to the log file, pipe-separated, one blank
    line between orders. Append mode only -- never overwrites prior
    orders. Returns (success, error_or_None).
    """
    try:
