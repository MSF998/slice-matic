...
test_core.py
------------
Plain-Python tests for core.pizzaflow_core (v2, cart model) -- no
Gradio required.

Run from the project root with:
    python tests/test_core.py

Covers all 8 required edge cases (re-mapped onto the cart model), cart
pricing math, file loading, and order persistence.
"""

import sys
import os

# Ensure project root is on sys.path when run directly from tests/
...
section("gr.Number sends Python floats/ints -- confirm those work the same way")
...
# topping quantity: blank is meaningful (= 0, not an error)
...
# In the v2 cart model, pizza/base/topping selection uses Gradio Dropdown
# components (valid choices only), so out-of-range index selection is not
# possible from user input. Quantity validation covers what remains.
...
# ----------------------------------------------------------------------
section("Edge case 8: menu file with a missing price field")
...
try:
    ...
    # append a second order, confirm first is preserved (not overwritten)
